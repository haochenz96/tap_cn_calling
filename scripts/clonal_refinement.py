...
def main(args):
    ...
    while(no_merge_candidate == False):
        merging_candidates = {}
        sorted_ids = np.sort(combined_NGT_df['clone_id'].unique())
        for i1 in range(len(sorted_ids)-1):
            for i2 in range(i1+1, len(sorted_ids)):
                clone1 = sorted_ids[i1]
        ...
        if len(merging_candidates) == 0:
            ...
            clone1, clone2 = list(merging_candidates.keys())[0]
            print(clone1, clone2)
            cn_assignment_df['clone_id'] = cn_assignment_df['clone_id'].replace(clone2, clone1)
